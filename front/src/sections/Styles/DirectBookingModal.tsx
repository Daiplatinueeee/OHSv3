...
import { MapPin, X, Check, ArrowLeft, Users, CheckCircle2, Gift } from "lucide-react"
import L from "leaflet"

import LocationSelectionModal from "../Styles/LocationSelectionModal"
...
const serviceLogo = "/service-logo.jpg"
...
interface DirectBookingModalProps {
  ...
  companyData: any // The pre-selected company data
}
...
function DirectBookingModal({ isOpen, onClose, serviceDetails, companyData }: DirectBookingModalProps) {
  console.log("[v0] DirectBookingModal: Component rendered. serviceDetails:", serviceDetails)
  console.log("[v0] DirectBookingModal: companyData:", companyData)
  ...
  const [, setHoveredSeller] = useState<Seller | null>(null)
  const [, setHoverPosition] = useState<{ x: number; y: number }>({ x: 0, y: 0 })
  ...
  const selectedSeller: Seller | null = companyData
    ? {
        id: companyData.id || companyData._id,
        name: companyData.businessName || companyData.name,
        firstName: companyData.firstName,
        lastName: companyData.lastName,
        middleName: companyData.middleName,
        businessName: companyData.businessName,
        totalRating: companyData.totalRating || 0,
        reviews: companyData.reviews || 0,
        location: companyData.location || "Online/Remote",
        startingRate: serviceDetails?.price || companyData.startingRate || 0,
        ratePerKm: serviceDetails?.chargePerKm || companyData.ratePerKm || 0,
        badges: companyData.badges || ["Company"],
        description: companyData.description || serviceDetails?.description || "",
        workerCount: serviceDetails?.workersNeeded || companyData.workerCount || 1,
        profilePicture: companyData.profilePicture,
        estimatedTime: serviceDetails?.estimatedTime || companyData.estimatedTime,
      }
    : null

  console.log("[v0] DirectBookingModal: selectedSeller created:", selectedSeller)
  ...
  }, [bookingStep, bookingSuccess, confirmationStep])
  ...
  useEffect(() => {
    if (selectedSeller) {
      setTotalRate(selectedSeller.startingRate)
      console.log("[v0] DirectBookingModal: Set initial total rate:", selectedSeller.startingRate)
    }
  }, [selectedSeller])

  if (!isOpen || !serviceDetails || !selectedSeller) {
    console.log("[v0] DirectBookingModal: Not rendering - missing data:", {
      isOpen,
  ...
  const selectLocation = (location: Location) => {
    ...
    if (selectedSeller) {
      ...
      console.log("[v0] DirectBookingModal: Updated total rate:", newTotalRate)
    }
  }

  // Modify the handleDateSelect function
  const handleDateSelect = (date: Date | undefined) => {
    if (date) {
      ...
      console.log("[v0] DirectBookingModal: Selected date:", formattedDate)
    } else {
      setSelectedDate("")
    }
  }
  ...
  const handleBookNow = () => {
    console.log("[v0] DirectBookingModal: Moving to confirmation step")
  ...
  const getUserAuthData = () => {
    try {
      ...
      console.log("[v0] DirectBookingModal: User data from localStorage:", userData)
      console.log("[v0] DirectBookingModal: Token from localStorage:", token)
  ...
  const submitBookingToDatabase = async () => {
    ...
    try {
      ...
      console.log("[v0] DirectBookingModal: Token being sent:", token)
      ...
      console.log("[v0] DirectBookingModal: Sending booking data:", bookingData)

      const apiUrl = `http://localhost:3000/api/bookings`
      console.log("[v0] DirectBookingModal: API URL:", apiUrl)
      ...
      console.log("[v0] DirectBookingModal: Booking created successfully:", result)
      ...
      try {
        ...
        console.log("[v0] DirectBookingModal: Creating booking notification:", notificationData)
  ...
  const handleConfirmBooking = async () => {
    console.log("[v0] DirectBookingModal: Starting booking confirmation")
    ...
    try {
      ...
      if (success) {
        console.log("[v0] DirectBookingModal: Booking successful")
        setIsSubmitting(false)
        setConfirmationStep(false)
        setBookingSuccess(true)
      } else {
        console.log("[v0] DirectBookingModal: Booking failed:", error)
        setIsSubmitting(false)
  ...
  const resetBooking = () => {
    console.log("[v0] DirectBookingModal: Resetting booking")
    setBookingStep(1) // Reset to step 1 instead of step 0
    setSelectedDate("")
  ...
  const getSellerName = (seller: any) => {
    console.log("[v0] DirectBookingModal: Seller object:", seller)
    console.log("[v0] DirectBookingModal: Available keys:", Object.keys(seller || {}))
    console.log("[v0] DirectBookingModal: businessName value:", seller?.businessName)

    if (seller?.name) {
      console.log("[v0] DirectBookingModal: Using seller.name:", seller.name)
      return seller.name
    }
    if (seller?.username) {
      console.log("[v0] DirectBookingModal: Using seller.username:", seller.username)
      return seller.username
    }
    if (seller?.businessName) {
      console.log("[v0] DirectBookingModal: Using seller.businessName:", seller.businessName)
    ...
    if (nameParts.length > 0) {
      ...
      console.log("[v0] DirectBookingModal: Constructed name:", constructedName)
    ...
    if (seller?.location) {
      ...
      return `Service Provider at ${mainLocation}`
    }

    console.log("[v0] DirectBookingModal: Using fallback: Service Provider")
        ...
        <div id="modal-content" className="overflow-y-auto flex-grow bg-white">
          {bookingStep === 1 && !confirmationStep && !bookingSuccess && (
            <div className="p-8 bg-gradient-to-br from-white to-gray-50/50 rounded-2xl shadow-xl max-w-4xl mx-auto">
              {/* Service Header - Changed from company header to service header */}
              <div className="mb-8">
                <div className="flex flex-col items-center gap-4 mb-6 text-center justify-center">
                  ...
                  <div className="flex-1">
                    <h3
                      className="text-[22px] font-medium text-gray-900 mb-2 cursor-pointer hover:text-sky-600 transition-colors"
                      onMouseEnter={(e) => handleSellerNameHover(selectedSeller, e)}
                      onMouseLeave={handleSellerNameLeave}
                    >
                      {serviceDetails.name}
                    </h3>
                    <p className="text-gray-600 leading-relaxed mb-2">{serviceDetails.description}</p>
                    <p className="text-sm text-gray-500">
                      Provided by <span className="font-medium text-sky-600">{getSellerName(selectedSeller)}</span>
                    </p>
                  </div>
                </div>
          ...
          {bookingStep === 2 && !confirmationStep && !bookingSuccess && (
            <div className="p-6">
              <div className="mb-4">
                ...
                <p className="text-gray-600 text-sm mb-2">{serviceDetails.description}</p>
          ...
          {bookingSuccess && (
            <div className="p-8 text-center">
              ...
              <p className="text-gray-600 mb-6">
                Stay tuned! {getSellerName(selectedSeller)} will review and accept your {serviceDetails.name} booking
                soon.
              </p>

              {/* Success details - same as WorkersModal */}
