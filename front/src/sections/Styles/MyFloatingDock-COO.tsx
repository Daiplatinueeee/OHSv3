...
const FloatingDock: React.FC = () => {
  ...
  const [, setCompletedBookingsCount] = useState(0);
  ...
  const fetchPendingBookingsCount = async () => {
    try {
      ...
      const response = await fetch("http://localhost:3000/api/bookings/all-pending", {
        headers: {
      ...
      if (response.ok) {
        ...
        // If there are no pending bookings, reset the notification flag
        if (pendingCount === 0) {
          setHasShownNotification(false)
  ...
  const fetchOngoingBookingsCount = async () => {
    try {
      const token = localStorage.getItem("token");
      if (!token) return;

      const response = await fetch("http://localhost:3000/api/bookings/ongoing", {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        const bookings = await response.json();
        const ongoingCount = bookings.length;

        // If there are no ongoing bookings, reset the notification flag
        if (ongoingCount === 0) {
          setHasShownNotification(false);
          setUnreadNotifications(0);
        }
        // Only show notification badge if we haven't shown it yet for this batch
          ...
          setUnreadNotifications(ongoingCount);
          setHasShownNotification(true);
        }
        // Keep showing the same count until it reaches 0
        else {
  ...
  const fetchActiveBookingsCount = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Reset notification if no active bookings
        if (activeCount === 0) {
          setHasShownNotification(false);
          setUnreadNotifications(0);
        }
        // Only trigger once per new active batch
        else if (!hasShownNotification) {
        ...
        console.error("Failed to fetch active bookings count.");
      }
    } catch (error) {
  ...
  const fetchCompletedBookingsCount = async () => {
    try {
      const response = await fetch("http://localhost:3000/api/bookings/completed");
      if (!response.ok) {
        console.error("Failed to fetch completed bookings:", response.statusText);
        return;
      }

      const data = await response.json();
      // setCompletedBookingsCount(data.length || 0);
      setCompletedBookingsCount(data.length || 0);
      console.log(`✅ Completed bookings count fetched: ${data.length}`);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    fetchOngoingBookingsCount()
    fetchActiveBookingsCount()
    fetchCompletedBookingsCount()
  }, [])

  // WARNING: This creates 60 requests per minute. Consider rate limiting implications.
  useEffect(() => {
